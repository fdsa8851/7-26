...
public class FoodDAO {
	...
	public static void updateInfo(FoodVO vo) throws Exception {
		Connection db = DBConn.getConnection();
		// 쿼리 날려서 유저 정보를 삽입
		// insert into user (name, email, username, pw) values ()
		String sql  = "update jejufood set address=?, name=?, bHours=?, food=? , holiday=?, groupf=? where u_idx=?";
		PreparedStatement pstmt = db.prepareStatement(sql);
		pstmt.setString(1, vo.getAddress());
		pstmt.setString(2, vo.getName());
		pstmt.setString(3, vo.getbHours());
		...
		System.out.println(vo.getbHours());
		System.out.println(vo.getFood());
		System.out.println(vo.getHoliday());
		System.out.println(vo.getGroupf());
	...
	public static void delInfo(String u_idx) throws Exception {
		...
		String sql  = "delete from jejufood where u_idx = ?";
		PreparedStatement pstmt = db.prepareStatement(sql);
	...
	public static FoodVO getFoodInfo(String u_idx) throws Exception {
		// DB 접속
		Connection db = DBConn.getConnection();
		// 쿼리 날려서 유저 정보를 검색
		String sql  = "select * from jejufood where u_idx = ?";
		PreparedStatement pstmt = db.prepareStatement(sql);
		pstmt.setString(1, u_idx);
		
		FoodVO vo = null;   // 사용자 정보를 담는 객체
