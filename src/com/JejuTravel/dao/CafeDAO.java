...
public class CafeDAO {
	public static ArrayList<CafeVO> getCafe() throws Exception {
		...
		while (rs.next()) {
			...
			CafeList.add(vo);   // 사용자 정보 객체를 리스트에 담기
		}	
		db.close();  // db 연결 정보 닫기
		return CafeList;   // 사용자 정보 리스트를 메소드 외부로 보내기
	}
	
	public static void InsertUser(CafeVO vo) throws Exception {
		// DB 접속
		Connection db = DBConn.getConnection();
		// 쿼리 날려서 유저 정보를 삽입
		// insert into user (name, phone, email, pw) values ('이름', '010-1234-1234', 'a@a.com', '1234')
		String sql  = "insert into jejucafe (address, name, bHours, food, holiday, groupf) values (?, ?, ?, ?, ?, ?)";
	...
	public static void updateInfo(CafeVO vo) throws Exception {
		Connection db = DBConn.getConnection();
		// 쿼리 날려서 유저 정보를 삽입
		// insert into user (name, email, username, pw) values ()
		String sql  = "update jejucafe set address=?, name=?, bHours=?, food=? , holiday=?, groupf=? where u_idx=?";
		PreparedStatement pstmt = db.prepareStatement(sql);
		pstmt.setString(1, vo.getAddress());
		pstmt.setString(2, vo.getName());
		pstmt.setString(3, vo.getbHours());
	...
	public static void delInfo(String u_idx) throws Exception {
		...
		String sql  = "delete from jejucafe where u_idx = ?";
		PreparedStatement pstmt = db.prepareStatement(sql);
	...
	public static CafeVO getCafeInfo(String u_idx) throws Exception {
		// DB 접속
		Connection db = DBConn.getConnection();
		// 쿼리 날려서 유저 정보를 검색
		String sql  = "select * from jejucafe where u_idx = ?";
		PreparedStatement pstmt = db.prepareStatement(sql);
		pstmt.setString(1, u_idx);
		
		CafeVO vo = null;   // 사용자 정보를 담는 객체
